...
class Interpreter(InterpreterBase):
    ...
    def __run_statements(self, statements, referenced_obj = None, this_obj = None):
        self.env.push()
        for statement in statements:
            ...
            if statement.elem_type == InterpreterBase.FCALL_DEF:
                self.__call_func(statement, referenced_obj)
            elif statement.elem_type == InterpreterBase.MCALL_DEF:
                self.__call_obj_func(statement, this_obj)
            elif statement.elem_type == "=":
                self.__assign(statement, referenced_obj)
    ...
    def __call_func(self, call_ast, referenced_obj = None):
        func_name = call_ast.get("name")
        if func_name == "print":
            return self.__call_print(call_ast, referenced_obj)
        if func_name == "inputi":
            return self.__call_input(call_ast, referenced_obj)
    ...
    def __call_obj_func(self, call_ast, referenced_obj = None):
        func_name = call_ast.get("name")
        if func_name == "print":
        ...
        if object is None:
            super().error(ErrorType.NAME_ERROR, f"not an object")
        try:
            if object.value().type != Type.OBJECT:
                super().error(ErrorType.TYPE_ERROR, f"not an object")
        except: 
            super().error(ErrorType.TYPE_ERROR, f"not an object")
        try:
            target_closure = object.value().get_thing(func_name).value()
            random_obj = object
        except: 
            super().error(
                ErrorType.NAME_ERROR, f"prototype func not found"
            )
        if target_closure == None:
            super().error(ErrorType.NAME_ERROR, f"Function {func_name} not found")
        try:    
            ...
            super().error(ErrorType.TYPE_ERROR, f"calling a method badly.")
        target_ast = target_closure.func_ast
        ...
        new_env["this"] = object
        self.env.push(new_env)
        _, return_val = self.__run_statements(target_ast.get("statements"), object, random_obj)
        self.env.pop()
        return return_val
    ...
    def __call_print(self, call_ast, referenced_obj = None):
        output = ""
        for arg in call_ast.get("args"):
            result = self.__eval_expr(arg, referenced_obj)  # result is a Value object
            output = output + get_printable(result)
        super().output(output)
        return Interpreter.NIL_VALUE

    def __call_input(self, call_ast, referenced_obj):
    ...
    def __assign(self, assign_ast, referenced_obj = None):
        var_name = assign_ast.get("name")
        parts = var_name.split(".")
        if len(parts) > 1:
            #means that it's an object method
            if parts[0] == "this":
                object = referenced_obj
            elif parts[1] == "proto":
                object = self.env.get(parts[0])
                src_value_obj = (self.__eval_expr(assign_ast.get("expression")))
                if src_value_obj.type() == Type.INT or src_value_obj.type() == Type.STRING or src_value_obj.type() == Type.BOOL:
                    super().error(
                        ErrorType.TYPE_ERROR, "Prototype cannot be a primitive"
                    )
                object.value().set_proto(src_value_obj.value())
            else:
                object = self.env.get(parts[0])
            obj_thing = parts[1]
            src_value_obj = (self.__eval_expr(assign_ast.get("expression")))
            try:
                if object.t == Type.CLOSURE and src_value_obj.t != Type.CLOSURE:
                    object.v.type = src_value_obj.t
            except:
                if src_value_obj.type() == Type.OBJECT:
                    src_value_obj = src_value_obj.value()
            if object.type() != Type.OBJECT:
                super().error(
                    ErrorType.TYPE_ERROR, "No inputi() function that takes > 1 parameter"
                )
            else: 
                object.value().set_thing(obj_thing, src_value_obj)
        else:
    ...
    def __eval_expr(self, expr_ast, referenced_obj = None):
        ...
        if expr_ast.elem_type == InterpreterBase.VAR_DEF:
            return self.__eval_name(expr_ast, referenced_obj)
        ...
        if expr_ast.elem_type == Interpreter.MCALL_DEF:
            #only one arg here!
            return self.__call_obj_func(expr_ast)
        if expr_ast.elem_type == Interpreter.THIS_DEF:
            print("d2")


    def __eval_name(self, name_ast, referenced_obj = None):
        ...
        if len(parts) > 1:
            if parts[0] == "this":
                if parts[1] is not None:
                    if parts[1] == "proto":
                        temp = self.env.get("this").value().get_proto()
                        val = Value(Type.OBJECT, temp)
                    else: 
                        val = self.env.get("this").value().get_thing(parts[1])
                elif referenced_obj is not None:
                    val = referenced_obj.value().get_thing(parts[1])
                else:
                    val = self.env.get("this")
            else: 
                object = self.env.get(parts[0])
                val_name = parts[1]
                if object.type() != Type.OBJECT:
                    super().error(
                        ErrorType.TYPE_ERROR, f"Variable/function {var_name} not found"
                    )
                #throw an error here if not an object
                val = (object.value().get_thing(val_name))
        if val is not None:
            return val
    ...
    def __eval_op(self, arith_ast):
        ...
        #what if one of these are an object?
